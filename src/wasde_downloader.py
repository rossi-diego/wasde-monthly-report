...
from config import RAW_DATA, WASDE_FOLDER
...
def extract_soybean_outlook_structured(xml_path):
    ...
    for report in soup.find_all("Report"):
        ...
        for region_group in report.find_all(re.compile(r"m\d+_region_group\d*")):
            ...
            for month_group in region_group.find_all(
                re.compile(r"m\d+_month_group\d*")
            ):
                for attr_group in month_group.find_all(
                    re.compile(r"m\d+_attribute_group\d*")
                ):
...
def extract_all_marketing_years(xml_path, commodities=None):
    ...
    for report in soup.find_all("Report"):
        ...
        matched = next(
            (c for c in commodities if c in title or "outlook" in title), None
        )
        if not matched:
            continue
...
def process_all_wasde_files():
    ...
    for date, xml_file in latest_by_date.items():
        ...
        try:
            ...
            df_combined = (
                pd.concat([df_main, df_soy], ignore_index=True)
                if not df_soy.empty
                else df_main
            )
